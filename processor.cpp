...
float processor::utilization() {
  ...
  if (prev_user == -1 || prev_nice == -1 || prev_system == -1 ||
      prev_idle == -1 || prev_iowait == -1 || prev_irq == -1 ||
      prev_softirq == -1 || prev_steal == -1) {
    prev_user = usertime;
    prev_nice = nicetime;
    ...
    const long int PREV_NON_IDLE = prev_user + prev_nice + prev_system +
