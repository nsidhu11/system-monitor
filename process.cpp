...
{
    vector<string> cpu_usage = linux_interpreter::process_Cpu_Utilization(this->pid);
    ...
    const float TOTAL_TIME = (u_time + s_time + cu_time + cs_time) / sysconf(_SC_CLK_TCK);
...
long int Process::up_Time()
{
    return linux_interpreter::up_Time(this->pid);
}
